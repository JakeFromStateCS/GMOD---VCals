...
function d_findVariable(s, i, len, lastType, jobstate)
    ...
    while true do
        --    Stop at the end. Throw an error. This function MUST NOT meet the endnot
        if i > len then
...
function s_anyVariable(data, lastType, isNumeric, isKey, isLast, jobstate)
    ...
    if lastType ~= tp then
        ...
        if _serialize[lastType] then
            ...
            error("vON: No serializer defined for type \"" .. lastType .. "\"not ")
        end
    end
            ...
            if e then
                ...
                if id then
                    if jobstate[1][id] and not jobstate[2] then
                        error("vON: There already is a table of reference #" .. id .. "not  Missing an option maybe?")
                    end
                    ...
                    error("vON: Malformed tablenot  Reference ID starting at char #" .. i .. " doesn't contain a numbernot ")
                end
            else
                error("vON: Malformed tablenot  Cannot find end of reference ID start at char #" .. i .. "not ")
            end
        end
...
--    Just a number which points to a table.
    ["table_reference"] = function(s, i, len, unnecessaryEnd, jobstate)
        ...
        if a then
            ...
            if n then
                return jobstate[1][n] or error("vON: Table reference does not point to a (yet) known tablenot "), a - 1
            else
                error("vON: Table reference definition does not contain a valid numbernot ")
            end
        end
...
--    Has various endings: \n, }, not , : and ;, some of which will force the table deserializer to go one char backwards.
    ["number"] = function(s, i, len, unnecessaryEnd, jobstate)
        ...
        if a then
            return tonumber(sub(s, i, a - 1)) or error("vON: Number definition does not contain a valid numbernot "), a - 1
        end
...
--    table.concat works better than normal concatenations WITH LARGE-ISH STRINGS ONLY.
    ["table"] = function(data, mustInitiate, isNumeric, isKey, isLast, first, jobstate)
        ...
        for k, v in next, data do
            ...
            end                                                                --    k % 1 not = 0 is the fastest way to check if a number
